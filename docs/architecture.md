...
- **No Candidate Execution Authority**: Milestone F stores candidate/dataset/cycle/evaluation artifacts only below `.agents/dream-data/policy-lab/`. It has no import path into online hooks, no active-policy write path, and no Shadow/Canary authority. Candidate activation starts only in a later reviewed milestone.
- **Zero Context Pollution**: Raw discovery histories, prompts, terminal logs, file/web contents, and replay artifacts are never injected into normal online model context; the policy designer receives only a sanitized aggregate packet.
