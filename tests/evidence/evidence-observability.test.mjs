...
} from "../../runtimes/antigravity/.agents/skills/orchestra/evidence-inspector.mjs";
...
test("evidence observability: project inspector is read-only and reports current git HEAD", () => {
  ...
  try {
    ...
    const after = JSON.stringify(JSON.parse(
      requireRead(join(root, ".agents", "state", "active-state.json"))
    ));
    ...
    assert.match(output, /Orchestra Evidence Status/);
